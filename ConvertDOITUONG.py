...
file_import = os.path.join(current_dir, 'Import_Excel_DanhMucDoiTuong.xlsx')
file_data = os.path.join(current_dir, 'Data_DoiTuong.xlsx')
...
required_columns = ['MaDoiTuong', 'MaHeThong']
...
df_data['MaDoiTuong'] = df_data['MaDoiTuong'].astype(str).str.strip()
df_data['MaHeThong'] = df_data['MaHeThong'].astype(str).str.strip()

# Tạo từ điển ánh xạ (loại bỏ giá trị nan)
mapping_dict = df_data.set_index('MaDoiTuong')['MaHeThong'].to_dict()
...
output_file = os.path.join(current_dir, 'Import_DoiTuong_Converted.xlsx')
